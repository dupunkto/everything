<?php
  ...
  if(!defined('GIT_SHA')) {
    fail("Updates are only available when running from a Git clone.", status: 404);
  }
  ...
  if($action == 'install') {
    if(!git_worktree_is_clean()) {
      fail("The update cannot be installed while the working tree has changes.", status: 409);
    }
